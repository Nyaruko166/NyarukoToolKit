...
public class Table {
    ...
    private String getSeparator() {
        ...
        switch (style) {
            ...
            case STYLE_ADVANCED ->
                    width = padSum + fontMetrics.charWidth('|') * header.size() + fontMetrics.charWidth(' ') * header.size();
            case STYLE_MORE_BORDER ->
                    width = padSum + fontMetrics.charWidth('|') * (header.size() + 1) + fontMetrics.charWidth(' ') * header.size();
        }
        if (width == 0) return "";
